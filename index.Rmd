---
title: "PM566 Final Project(Assignment5)"
author: "Suyeon Ryu"
output: 
    html_document:
        toc: TRUE
...
For Assignment 5, I have included 3 interactive figures(2 plots / 1 map)
...
Overall, the trends in colorectal cancer incidence rate shows decreasing trend over the period of 18 years (1999 to 2017). The rate decreasing slope was steeper until early 2000, then shows stablizing trend from 2007 to 2017 (Figure 1). Regardless, the overall mortality from colon cancer in U.S. maintained around same rate over 17 years, without showing particularly decreasing trends (Figure 2). 
...
ggplot(cln_inc, aes(x=YEAR, y=CRUDE_RATE))+
  ...
  ggtitle("FIGURE 1.Overall U.S. Colon Cancer Incidence Crude Rate")

ggplot(cln_mort, aes(x=YEAR, y=CRUDE_RATE))+
  geom_point(color="red")+
  theme_minimal()+
  geom_smooth(method="loess", color="red", fill="darkred")+
  ggtitle("FIGURE 2. Overall U.S. Colon Cancer Mortality Crude Rate")


```


### (Assignment5) 2 x INTERACTIVE PLOTS for Mortalities and Incidence in Colorectal Cancer
...
### (Assignment5) INTERACTIVE MAP for Considered States and Their Mortality Rates
...
df <- cln_mort %>% select(code, POPULATION,AGE_ADJUSTED_RATE) # select data
df <- subset(df, df$AGE_ADJUSTED_RATE < 100)

df$hover <- with(df, paste(code, "Age adjusted rate", '<br>',df$POPULATION,'<br>',"Population"))
...
ggplot(data=cln3, aes(x=YEAR, y=CRUDE_RATE, color=westeast, shape=westeast)) +
  ...
  ggtitle("FIGURE 5. U.S. Colon Cancer Incidence Crude Rate West Coast vs. East Coast")
...
```{r,echo=FALSE, error=FALSE, message = FALSE,eval=FALSE}
...
ggplot(data=cln4, aes(x=YEAR, y=AGE_ADJUSTED_RATE, color=westeast, shape=westeast)) +
  ...
  scale_size_manual(values=c(7,7))+
  theme(legend.position="right")+
  ggtitle("FIGURE 6. U.S. Colon Cancer AGE Adjusted Rate West Coast vs. East Coast")
...
ggplot(data=mort3, aes(x=YEAR, y=CRUDE_RATE, color=westeast, shape=westeast)) +
  ...
  scale_size_manual(values=c(7,7))+
  theme(legend.position="right")+
  ggtitle("FIGURE 7. U.S. Colon Cancer Mortality Crude Rate West Coast vs. East Coast")
...
ggplot(data=mort3, aes(x=YEAR, y=AGE_ADJUSTED_RATE, color=westeast, shape=westeast)) +
  ...
  scale_size_manual(values=c(7,7))+
  theme(legend.position="right")+
  ggtitle("FIGURE 8. U.S. Colon Cancer AGE Adjusted Mortality Rate West Coast vs. East Coast")
